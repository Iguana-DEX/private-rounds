...
export function createTotalEthPledgedChangedEvent(
  groupAddress: Address,
  roundId: BigInt,
  totalEthPledged: BigInt
): TotalEthPledgedChanged {
  let totalEthPledgedChangedEvent = changetype<TotalEthPledgedChanged>(
    newMockEvent()
  )

  totalEthPledgedChangedEvent.parameters = new Array()

  totalEthPledgedChangedEvent.parameters.push(
    new ethereum.EventParam(
      "groupAddress",
      ethereum.Value.fromAddress(groupAddress)
    )
  ...
  totalEthPledgedChangedEvent.parameters.push(
    new ethereum.EventParam(
      "totalEthPledged",
      ethereum.Value.fromUnsignedBigInt(totalEthPledged)
    )
  )

  return totalEthPledgedChangedEvent
}
